...
import com.user.festivalbizerte.Adapter.SponsorAdapter;
...
public class SponsorActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    Context context;
    @BindView(R.id.recyclesponsor)
    ...
    private void loadSponsor() {
        Call<RSResponse> callUpload = WebService.getInstance().getApi().loadSponsor();
        callUpload.enqueue(new Callback<RSResponse>() {
            @Override
            public void onResponse(Call<RSResponse> call, Response<RSResponse> response) {
                if (response.body() != null) {
                    if (response.body().getStatus() == 1) {
                        Sponsors[] tab = new Gson().fromJson(new Gson().toJson(response.body().getData()), Sponsors[].class);
                        ListSponsor = Arrays.asList(tab);
                        sponsorAdapter = new SponsorAdapter(context, ListSponsor);
                        RecyclerviewSponsor.setAdapter(sponsorAdapter);
                        RecyclerviewSponsor.setLayoutManager(new LinearLayoutManager(context));

                    } else if (response.body().getStatus() == 0) {
                        Toast.makeText(getApplicationContext(), "Pas de Sponsor ", Toast.LENGTH_SHORT).show();
                    }
                }
            }
